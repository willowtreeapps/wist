...
/**
 * Creates the .bslintrc.json file in the default location if a configuration path is not specified.
...
function handleInitialize(currentOptions) {
    const recommendedFilePath = '../config/bslint-recommended.json';
    let result = 0;
...
/**
 * Creates the .bslintrc.json file from the user-defined configuration file.
...
/**
 * Creates the .bslintrc.json file from the contents of the specified file.
 * @param configFilePath Path to the configuration file.
 * @returns {boolean} True if the creation of the configuration file succeeds, false if not.
 * @private
 */
function setupConfigurationFile(configFilePath) {
    const fileName = '.bslintrc.json';
