        ...
        return {
            'functionDeclaration:exit': function(node) {
                ...
                if (children.length === 0) {
                    ...
                    const endOfDeclaration = node.children[node.children.findIndex(child => child.ruleIndex === Parser.rule.endOfStatement) - 1];

                    context.report({
                        message: `Missing return statement for function '${funcName}'`,
                        loc: {
                            start: node.start,
                            end: endOfDeclaration.stop
                        }
                    });
                }
            },
            'anonymousFunctionDeclaration:exit': function(node) {
                ...
                if (children.length === 0) {
                    ...
                    node.start.column = node.start.column + 1;
                    endOfDeclaration.stop.column = endOfDeclaration.stop.column + 1;
