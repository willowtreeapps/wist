...
class Config {
    ...
    */
    ...
    getConfigHierarchy(directory) {
        ...
        // Step 2: Add user-specified config from .wisteriarc.* files
    ...
    getLocalConfigHierarchy(directory) {
        ...
        if (!configs.length && !this.specificConfig) {
            // Fall back on the personal config from ~/.wisteriarc
            ...
            if (personalConfig) {
                ...
                const noConfigError = new Error('No Wisteria configuration found.');
     ...
     * vector entry { configFile: '/home/john/app/.wisteriarc', matchingOverrides: [0, 2] } would indicate that the main
     * project .wisteriarc file and its first and third override blocks apply to the current file.
     ...
     * that apply to the current file, including the base config (conf/wisteria-recommended), the user's personal config
