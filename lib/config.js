...
class Config {
    ...
    */
    ...
    getConfigHierarchy(directory) {
        ...
        // Step 2: Add user-specified config from .bslintrc.* files
    ...
    getLocalConfigHierarchy(directory) {
        ...
        if (!configs.length && !this.specificConfig) {
            // Fall back on the personal config from ~/.bslintrc
            ...
            if (personalConfig) {
                ...
                const noConfigError = new Error('No BSLint configuration found.');
     ...
     * vector entry { configFile: '/home/john/app/.bslintrc', matchingOverrides: [0, 2] } would indicate that the main
     * project .bslintrc file and its first and third override blocks apply to the current file.
     ...
     * that apply to the current file, including the base config (conf/bslint-recommended), the user's personal config
